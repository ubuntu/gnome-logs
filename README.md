...
git remote add ubuntu https://github.com/ubuntu/gnome-logs.git
git fetch --all
```
...
5. Once the PR has been merged into the stable branch, then on the next git mirror sync (every 4 hours), launchpad will trigger [a build of the snap that gets published](https://code.launchpad.net/~desktop-snappers/gnome-logs/+git/snap/+ref/stable) to the [snap store](https://snapcraft.io/gnome-logs) into the *candidate* channel. After sufficient testing of the snap from the candidate channel, then the reviewer (a Collaborator of the snap in the store) will promote the snap to the stable branch in the snap store.
